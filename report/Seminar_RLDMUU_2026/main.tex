...
The mean approximation ratio of $\rho = 1.02$ (std $= 0.09$) indicates that linear contracts recover approximately $98\%$ of the principal’s utility on average, far below the theoretical worst-case bound of $2$. This aligns with the result of \citet{dutting2021binary}, showing that in binary-outcome settings where one outcome has zero reward, linear contracts are optimal ($\rho = 1$). Since our environments retain a similar low-dimensional reward structure, the empirical ratios remain very close to $1$.
