...
\section{Algorithms}
\textbf{Environment}\\
In a first setting, the implemented algorithms perform over a simple initialization of the hidden-action principal-agent MDP as presented in the paper \cite{ivanov2024principal}.  
\begin{figure} [H]
    \centering
...
\end{figure}
The MDP has three states $S=\{s_0,s_L,s_R\}$, at each the agent can chose between two actions $A=\{a_L,a_R\}$ who can lead to a specific outcome $O=\{L,R\}$. The distribution of outcome L taking action $a_L$ is of 0.9, while taking action $a_R$ it is of 0.1; while the distribution of outcome R is of 0.9 when taking action $a_R$ and of 0.1 when taking action $a_L$.\\ 
Agent and principal have different rewards for a certain outcome at state $s \in S$. At $(s,L)$ the principal has reward $14\over9$, while the agent has reward -$4\over5$ making it costly. At $(s,R)$ both the agent and the principal have reward $0$. 

\subsection{Meta-algorithm}
\label{subsec:meta_algorithm}
The first implemented algorithm performs iteratively optimization over the policies of the agent and of the principal in their respective MDPs. Both actors have complete knowledge over the MDPs, meaning that they can access all their elements including transition and reward functions.\\
The given name comes from the fact that the learning that would usually be applied during inner and outer optimizations is not yet specified, in our case we perform value iteration.


Meta-algorithm converges to subgame-perfect equilibrium
